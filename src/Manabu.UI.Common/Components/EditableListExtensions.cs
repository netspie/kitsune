...
{
    public static string GetIndexStr(this EditableList.TreeItemData item)
    {
        return item.GetIndexes()
            .Select(i => (i + 1).ToString())
            .AggregateOrDefault((x, y) => $"{x}.{y}");
    }

    public static int[] GetIndexes(this EditableList.TreeItemData item) =>
        item.Flatten(i =>
        {
            if (i?.Parent is null)
